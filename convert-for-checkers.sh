...
if [ -f $1 ]; then
  ...
  ${Aiger:-false} && {
    ...
    $AUTOMATA_SAFA_ONE share < $f.afa | $AUTOMATA_SAFA_ONE removeUselessShares > /tmp/afa.afa
    if $AUTOMATA_SAFA_ONE hasComplexFinals < /tmp/afa.afa; then
      $AUTOMATA_SAFA_ONE removeFinals < /tmp/afa.afa | $LTLE_TO_AFA prettyToSmv | $SMVTOAIG > $f.aig
    else
      $LTLE_TO_AFA prettyToSmv < /tmp/afa.afa | $SMVTOAIG > $f.aig
    fi
  }
  ${AigerSimp:-false} && {
    echo "Transforming to .aig"
