...
convertFormulae formulae fcount qcount symCount = runST action
  where
    ...
    action = do
      ...
      ((ixMap0, fbterms0), fmterms0) <- runNoConsTFrom qcount $
        runNoConsTFrom (4 + symCount) $
          ($ formulaeArr) $
            cataScanT' @(LLSTArray s) ftraversed $
              \f -> do
                let f' = ($ f) $
                      cataRecursive $ \case
                        STrue -> Left $ Pure 0
...
toAfa args@(qcount, fcount, init, final, formulae, states) =
  unswallow
    BoolAfa
      { boolTerms =
          listArray' $
            ...
            , Free $ BTerm.Not $ Pure 2
              ...
              ++ map (Free . BTerm.Predicate) [qcount + 1 .. qcount + symCount]
              ++ fbterms
      , Afa.Bool.afa =
          Afa
  ...
  where
    ...
    ((states', formulae'), length -> symCount) =
      runIdentity $
        runHashConsTFrom 4 $
    ...
    init' = convertTransition (runIdentity $ vtraversed rejectVar init) >>= Free . MTerm.State
      where
        rejectVar = error "variable in init formula"
...
formatAfa (BoolAfa bterms (Afa mterms states init)) =
  T.unlines
