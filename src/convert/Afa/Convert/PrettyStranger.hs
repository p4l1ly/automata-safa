...
convertFormulae :: [Fix STerm'] -> Int -> Int -> Int
  -> ( Array Int (Bool, Int)
     , [Free (BTerm.Term Int) Int]
     , [Free (MTerm.Term (Free (BTerm.Term Int) Int) Int) Int]
     )
convertFormulae formulae fcount qcount acount = runST action where
...
toAfa :: Int -> Int -> Int -> Fix STerm' -> Fix STerm' -> [Fix STerm'] -> [Fix STerm']
  -> BoolAfaUnswallowed Int
toAfa qcount fcount acount init final states formulae =
  traceShow (qcount, fcount, acount, init, final, formulae, states)$
  unswallow$ (\x -> traceShow x x)$ BoolAfa
  { boolTerms = listArray'$
      [ Free BTerm.LTrue
      , Free BTerm.LFalse
      , Free$ BTerm.Predicate qcount
      , Free$ BTerm.Not$ Pure 2
      ] ++ map (Free . BTerm.Predicate) [qcount + 1 .. qcount + acount] ++ fbterms
  , Afa.Bool.afa = Afa
  ...
  extendedStates
    | and simpleNonfinal && isNothing nonsimpleFinal = traceShow "simsimsim"$
        [0..qcount - 1] <&> Free . MTerm.State
    | otherwise = [0..qcount - 1] <&> \i ->
...
finalSplitSimple qcount final = traceShow (qcount, final)$
...
mtfol0 = MTFol (\_ -> mempty) (\_ -> mempty) (\_ -> mempty) (\_ -> mempty)
...
{-# INLINE appMTTra #-}
appMTTra :: forall m q f v r q' f' v' r'. Applicative m
  => MTTra q f v r q' f' v' r' m -> STerm q f v r -> m (STerm q' f' v' r')
