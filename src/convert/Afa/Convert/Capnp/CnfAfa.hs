...
import qualified Capnp.Gen.Schema.CnfAfa.Pure as Schema
import System.IO
...
serializeCnfAfa :: CnfAfa -> Schema.CnfAfa
serializeCnfAfa (CnfAfa states varCount clauses) = Schema.CnfAfa
  { Schema.variableCount = fromIntegral varCount
  , Schema.outputs = V.fromList$ map toSchemaLit$ elems states
  , Schema.clauses = V.fromList$ map (V.fromList . map toSchemaLit)$ elems clauses
  }
