    ...
    traversal residualGraphM arr rec = \case
      (Visited pp, i) -> do
        ...
        lift (unsafeRead residualGraphM i) >>= \case
          ...
          Just _ -> return ()
          _
            | sinkFlags `unsafeAt` i -> throwE $ pp `appEndo` [i]
        ...
        where
          ...
          recBackDown back j = unless (sourceFlags `unsafeAt` j) $ rec'' j
            where
              rec'' = rec . (Visited $ pp <> Endo (\xs -> i : back : xs),)
      _ -> return ()

{-# INLINEABLE minCut #-}
minCut :: Traversable f => Array Int (f Int) -> [Int] -> [Int] -> [Int]
minCut nodes sources sinks = runST action
  where
    action :: forall s. ST s [Int]
    action = do
      let marks = isJust <$> maxFlow nodes sources sourceFlags sinkFlags
      let marks1 =
            ixedNodes & cataScan (_2 . traversed) %~ \(i, fb) ->
              marks `unsafeAt` i || or fb && not (sinkFlags `unsafeAt` i)

      let setter rec = \(g, i) ->
            let overmined = marks `unsafeAt` i || getAny g
             in overmined <$ for_ (nodes `unsafeAt` i) (rec . (Any overmined,))
    ...
    underSources = runST action
      where
        action :: forall s. ST s (Array Int Bool)
        action = do
          let setter rec = \(g, i) ->
                let overmined = sourceFlags `unsafeAt` i || getAny g
                 in overmined <$ for_ (nodes `unsafeAt` i) (rec . (Any overmined,))
          marks2Init :: STArray s Int Any <- newArray (bounds nodes) (Any False)
          marks2M :: STArray s Int Bool <- marks2Init & anaScanT2 setter return
...
minCut2Lowest nodes sources sinks =
  ...
  where
    ...
    maxIx = snd bnds
    revIx = (maxIx -)
    revNodes = ixmap bnds revIx $ runST action
      where
...
{-# INLINEABLE enumerateEdges #-}
enumerateEdges :: Foldable f => Array Int (f Int) -> ([Int], [Word])
enumerateEdges arr = runIdentity $
  runNoConsT $ do
    arr' <- for arr $ \edges ->
...
foreign import ccall "automata_safa.h min_cut_highest"
  min_cut_highest_ffi ::
    Word ->
    Ptr Word ->
    Word ->
...
foreign import ccall "automata_safa.h &free_min_cut_highest"
  free_min_cut_highest_ffi ::
    FunPtr (Ptr Word -> IO ())

{-# INLINEABLE minCut2HighestFFI #-}
          ...
          CA.withCArray edgesC $ \edgesPtr ->
            CA.withCArray sourcesC $ \sourcesPtr ->
              CA.withCArray sinksC $ \sinksPtr -> do
                cutSize <-
                  min_cut_highest_ffi
                    (fromIntegral $ CA.size nodesC)
                    nodesPtr
                    (fromIntegral $ CA.size edgesC)
                    edgesPtr
                    (fromIntegral $ CA.size sourcesC)
                    sourcesPtr
                    (fromIntegral $ CA.size sinksC)
                    sinksPtr
                    cutPtr
                cutPtr' <- peek cutPtr
