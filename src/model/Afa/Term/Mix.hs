...
childModToPlate ChildMod{lP, lQ, lT} =
  plateBase
...
{-# INLINE modChilds #-}
modChilds ::
  forall f p q t p' q' t'.
  Applicative f =>
  ChildMod p q t p' q' t' f ->
  Term p q t ->
  f (Term p' q' t')
...
fromMTFun MTFun{mtfunP, mtfunQ, mtfunT} =
  mfun0
...
fromMTFol MTFol{mtfolP, mtfolQ, mtfolT} =
  mfol0
...
fromMTTra MTTra{mttraP, mttraQ, mttraT} =
  mtra0
