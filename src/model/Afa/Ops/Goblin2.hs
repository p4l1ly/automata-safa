...
import Data.Array
import Data.Array.Base (numElements, unsafeAccumArray, unsafeAt, unsafeWrite)
...
newtype Builder s p a = Builder {runBuilder :: BuilderCtx s p -> ST s a}
  ...
  deriving
    ...
    via ReaderRef (Field "algDomain" "ctx" (MonadReader (ReaderT (BuilderCtx s p) (ST s))))
  deriving
...
extractAndQac ::
  ...
  Term p (Bool, Int) (Bool, (Mix, Int)) ->
  Builder s p (Mix, Int)
extractAndQac (qacEnv -> qac) = helper
  where
    helper (extract -> x) =
      fmap snd $
        flip iterM x $
...
goblin2 afa@(Afa terms states init) = Afa terms' states' init
  where
    ...
    myCata stageEnd stageStart = thisStage stageStart
      where
        thisStage i = do
