...
randomize (BoolAfa bterms (Afa mterms states init)) = do
  ...
  let shuffledBTerms =
      ...
      (btermPermut', listArray (bounds bterms) -> bterms') = runST action
        where
          ...
          action = runNoConsT $ do
            ...
            fmap (listArray (bounds bterms)) $
              for (range (bounds bterms)) $
                flip dfs gs $ \rec -> \case
                  (Just (First j), _) -> return j
                  (_, i) -> do
                    bterm' <-
                      BTerm.appMTTra BTerm.mttra0{BTerm.mttraT = rec . (Nothing,)} $
                        shuffledBTerms ! i
                    j <- nocons bterm'
  ...
  let shuffledMTerms =
      ...
      (mtermPermut', listArray (bounds mterms) -> mterms') = runST action
        where
          ...
          action = runNoConsT $ do
            ...
            fmap (listArray (bounds mterms)) $
              for (range (bounds mterms)) $
                flip dfs gs $ \rec -> \case
                  (Just (First j), _) -> return j
                  (_, i) -> do
                    mterm' <-
                      MTerm.appMTTra MTerm.mttra0{MTerm.mttraT = rec . (Nothing,)} $
                        shuffledMTerms ! i
                    j <- nocons mterm'
                    unsafeWrite gs i $ Just $ First j
                    return j
