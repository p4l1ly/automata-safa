...
positiveOp :: (NonEmpty Int -> MTerm.Term Int Int Int)
  -> BoolAfaUnswallowed p -> BoolAfaUnswallowed p -> BoolAfaUnswallowed p
positiveOp op
  ...
  bterms = listArray (0, bterms1Count + bterms2Count - 1)$
  ...
  states = listArray (0, states1Count + states2Count)$
    elems states1
...
complementUnsafeDeep (BoolAfa bterms1 (Afa mterms1 states1 init1)) =
  ...
  where
  (ixMap, bterms) = runST action where
    action :: forall s. ST s (Array Int Int, [BTerm.Term p Int])
    action = runNoConsT$ cataScanT' @(LSTArray s) traversed alg bterms1 where
